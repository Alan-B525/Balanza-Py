...
# Esto evita bloqueos cuando se abre el wizard de calibraci├│n
try:
    import matplotlib
...
class BalanzaGUI(ttk.Window):
    def _load_calibration_session(self, parent=None):
        """Carga los puntos de calibraci├│n desde disco y refresca la UI."""
        try:
            if hasattr(self, '_cal_manager') and self._cal_manager:
                self._cal_manager.load_points()
                self.log_message("Sesi├│n de calibraci├│n cargada desde disco.")
                ...
                self.log_message("No existe un gestor de calibraci├│n activo.")
        except Exception as e:
            self.log_message(f"Erro ao carregar sess├úo de calibra├º├úo: {e}")
    ...
    def __init__(self, data_queue, command_queue, data_processor=None):
        ...
        # Si la resoluci├│n es 1280x800, quitar barra superior
        ...
        # ├Ültimo timestamp visto por widget para cada sensor (mantener display hasta nueva muestra)
        self._widget_last_seen = {}
        # ├Ültimo timestamp visto para el total (mantener total hasta nueva muestra)
        self._widget_last_total = 0.0
        
        # Control de visualizaci├│n de decimales (por defecto: SIN decimales)
        ...
        # Iniciar conexi├│n autom├ítica al arrancar (mostrar di├ílogo de conexi├│n)
        try:
    ...
    def _calculate_scale_factors(self, screen_width, screen_height):
        """Calcula factores de escala basados en la resoluci├│n de pantalla."""
        # Factor de escala (relativo a la resoluci├│n base 1280x800)
        self.scale_x = screen_width / BASE_WIDTH
        self.scale_y = screen_height / BASE_HEIGHT
        
        # Factor de escala general (promedio geom├®trico para mantener proporciones)
        self.scale = min(self.scale_x, self.scale_y)
        
        # Factor espec├¡fico para fuentes (no escalar demasiado en pantallas grandes)
        # Limitar entre 0.8 y 1.5 para mantener legibilidad
        self.font_scale = max(0.8, min(1.5, self.scale))
        
        # Log en archivo
        self.log_message(f"[GUI] Resoluci├│n detectada: {screen_width}x{screen_height}")
        self.log_message(f"[GUI] Factor de escala: {self.scale:.2f} (fuentes: {self.font_scale:.2f})")
    
    def scaled(self, value):
        """Escala un valor num├®rico seg├║n la resoluci├│n."""
        return int(value * self.scale)
    
    def scaled_font(self, size):
        """Escala el tama├▒o de fuente seg├║n la resoluci├│n."""
    ...
    def _fit_label_font(self, label, text, family, max_size, min_size=8, weight='bold', explicit_width=None):
        """Ajusta el tama├▒o de fuente de `label` para que `text` quepa en su ancho disponible.

        - `explicit_width`: si se proporciona (en p├¡xeles), se usa en lugar de medir el widget.
        """
        try:
            import tkinter.font as tkfont
            # --- CORRECCI├ôN: NO LLAMAR A update_idletasks() ---
            ...
            if explicit_width and isinstance(explicit_width, int) and explicit_width > 0:
                ...
                # Si el widget a├║n no se pint├│, usar el requerimiento o un default seguro
                if avail_w <= 1:
                    avail_w = label.winfo_reqwidth()

            # Si a├║n as├¡ es muy peque├▒o, usar un default para evitar errores
            if avail_w <= 1: 
                avail_w = 200
            ...
            # B├║squeda simple para elegir tama├▒o
            chosen_size = min(max_size, max(min_size, int(max_size)))
            # Optimizaci├│n: probar primero el tama├▒o m├íximo
    ...
    def _configure_styles(self):
        ...
        # Fonts - Escalados seg├║n resoluci├│n
        FONT_MAIN = "Segoe UI"
        FONT_MONO = "Consolas"
        FONT_NUMBERS = "Segoe UI"  # Usar la misma fuente que TareMaintValue para todos los n├║meros
        
        # Funci├│n helper local para escalar fuentes
        sf = self.scaled_font
        ...
        # Mantener una versi├│n monoespaciada para el total de la pesta├▒a de mantenimiento
        ...
        # Tabs config - Pesta├▒as gruesas, anchas y centradas
        ...
        # Reducir tama├▒o del t├¡tulo para que no ocupe tanto espacio en el footer
    ...
    def _setup_ui(self):
        ...
        # Intentar cargar un solo logo principal junto al t├¡tulo
        import os, sys
        base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        assets_path = os.path.join(base_path, "assets")

        self.logo_img = None
        # Tama├▒o de logo (escalado)
        logo_height = self.scaled(80)
        # Guardar resample_method solo si PIL est├í disponible
        ...
        # Header: mostrar t├¡tulo y estado en la parte superior (no en el footer)
        try:
            ...
            self._status_led = ttk.Label(brand_frame, text="ÔùÅ", font=("Segoe UI", self.scaled_font(42)))
            ...
            try:
                ...
                pass
            # Compatibilidad con c├│digo existente que usa self.lbl_status
            self.lbl_status = self._status_text
        except Exception:
            pass

        # Establecer icono de la aplicaci├│n (barra de tareas + t├¡tulo)
        try:
        ...
        ttk.Frame(actions_frame, width=20, style='Header.TFrame').pack(side=LEFT)
        ...
        # Ocultar el bot├│n de decimales en todas las resoluciones: no hacer pack()

        # (Export/Import moved to Config -> CALIBRACAO tab)
        
        # Bot├│n de TARA en el header (mismo formato, color amarillo)
        try:
        ...
        ttk.Frame(actions_frame, width=10, style='Header.TFrame').pack(side=LEFT)
        ...
        # Nota: el t├¡tulo y el estado se muestran en el pie (footer)
        ...
        # Footer frame fijo (creado antes del grid central para garantizar visibilidad en pantallas peque├▒as)
        try:
            footer_frame = ttk.Frame(main_container, style='Header.TFrame')
            footer_frame.pack(side=BOTTOM, fill=X, pady=(4, 4))

            # Caja para t├¡tulo + estado (alineados a la izquierda)
            footer_left = ttk.Frame(footer_frame, style='CardNoBorder.TFrame')
            footer_left.pack(side=LEFT, anchor='w', padx=(12, 6), pady=(6, 4))

            # T├¡tulo del sistema (mostrar en el pie)
            try:
                from config import APP_TITLE
            except Exception:
                APP_TITLE = "Sistema de Pesagem"
            try:
                # No mostrar t├¡tulo duplicado aqu├¡; mantener compatibilidad con self.lbl_status
                # Si no existe self.lbl_status (header), crear una referencia m├¡nima sin pack.
            ...
            # Cargar ambos logos en el footer con el mismo tama├▒o
            try:
                try:
        ...
        # =====================================================================
        # NOTEBOOK PRINCIPAL CON 2 PESTA├æAS: VIGAS | SENSORES
        # =====================================================================
        # Estilo para pesta├▒as grandes y touch-friendly en la ventana principal
        style = ttk.Style()
        style.configure('MainTab.TNotebook', tabmargins=[0, 0, 0, 0])
        style.configure('MainTab.TNotebook.Tab',
                        font=('Segoe UI', self.scaled_font(18), 'bold'),
                        padding=(40, 14),
                        anchor='center',
                        justify='center',
                        background="#e2e8f0",
                        foreground="#475569")
        style.map('MainTab.TNotebook.Tab',
                  background=[('selected', '#2563eb')],
                  foreground=[('selected', 'white')])

        self._main_notebook = ttk.Notebook(main_container, style='MainTab.TNotebook')
        self._main_notebook.pack(fill=BOTH, expand=YES)

        # Ajustar ancho de pesta├▒as para que ocupen todo el ancho equitativamente
        def _resize_tabs(event=None):
            try:
                nb = self._main_notebook
                nb.update_idletasks()
                total_w = nb.winfo_width()
                n_tabs = nb.index('end')
                if n_tabs > 0 and total_w > 100:
                    # Calcular ancho por pesta├▒a en caracteres (aproximado)
                    # Usar un ancho grande fijo que garantice que se reparta
                    tab_w_chars = max(10, total_w // (n_tabs * 12))
                    style.configure('MainTab.TNotebook.Tab', width=tab_w_chars)
            except Exception:
                pass
        self._main_notebook.bind('<Configure>', _resize_tabs)

        # --- Tab 1: VIGAS (contenido actual) ---
        tab_vigas = ttk.Frame(self._main_notebook, style='Body.TFrame')
        self._main_notebook.add(tab_vigas, text="  VIGAS  ")

        grid_area = ttk.Frame(tab_vigas, style='Body.TFrame')
        grid_area.pack(fill=BOTH, expand=YES)

        # --- Tab 2: SENSORES (se configura despu├®s) ---
        self._tab_sensores = ttk.Frame(self._main_notebook, style='Body.TFrame')
        self._main_notebook.add(self._tab_sensores, text="  SENSORES  ")
        # CONFIGURACI├ôN CLAVE: 'uniform' obliga a que las columnas 0 y 2 midan ID├ëNTICO
        ...
        try:
            ...
            is_tablet = False
        # Reservar un m├¡nimo reducido para la fila inferior (ahora vac├¡a)
        tare_min = 0
        ...
        def create_static_beam_card(parent, title, col, sensor_nums=""):
            ...
            # BLOQUEO DE TAMA├æO: Evita que el contenido estire la tarjeta
            try:
            ...
            # ID(s) de nodo debajo del t├¡tulo
            if sensor_nums:
        ...
        try:
            ...
            # IDs ├║nicos (preservando orden) de los nodos de Viga 1
            _seen = set()
            _v1_ids_list = []
        ...
        # Inicializar diccionario vac├¡o de widgets de sensores individuales
        self.sensor_widgets = {}
        # Valor actual de tara conocida (toneladas)
        self._current_tare = 0.0
        # -------------------------------------------------------------
        # Secci├│n principal: CONTROL DE TARA (debajo de las tarjetas)
        # Dise├▒o: grid con 3 columnas (bot├│n izq | centro con t├¡tulo+valor | bot├│n der)
        # -------------------------------------------------------------
        # Secci├│n de TARA eliminada del layout principal: ahora el bot├│n TARA est├í en el header.

        # Configurar la pesta├▒a SENSORES (lecturas individuales de cada c├®lula)
        self._setup_sensores_tab(self._tab_sensores)
        
        
    def _setup_sensores_tab(self, parent):
        """Configura a aba SENSORES com cards individuais de cada c├®lula de carga."""
        import json, os
        from config import SETTINGS_FILE, NODOS_CONFIG

        # Limpiar contenido anterior (para re-inicializaci├│n tras cambio de config)
        ...
        current_config = {"nodes": NODOS_CONFIG}
        ...
        maint_grid = ttk.Frame(parent, style='Body.TFrame')
        maint_grid.pack(fill=BOTH, expand=YES)
        # Columnas est├íticas: izquierda | centro | derecha
        ...
        def create_sensor_card_main(parent_frame, key, title, row, col):
            card_w = self.scaled(280)
            card_h = self.scaled(180)
            card = ttk.Frame(parent_frame, style='Card.TFrame', padding=12)
        ...
        # Usar LabelFrame para vigas
        try:
            viga1_frame = ttk.Labelframe(maint_grid, text=f"VIGA 1 - N├ô: {v1_ids}", padding=6, labelanchor='n', style='Viga.TLabelframe')
        except Exception:
        ...
        try:
            viga2_frame = ttk.Labelframe(maint_grid, text=f"VIGA 2 - N├ô: {v2_ids}", padding=6, labelanchor='n', style='Viga.TLabelframe')
        except Exception:
        ...
        # Colocar cards de las celdas dentro de viga1_frame y viga2_frame
        ...
        for key, r in left_placements:
            if key in nodes_cfg:
                ...
                create_sensor_card_main(viga1_frame, key, title, r, 0)
        ...
        for key, r in right_placements:
            if key in nodes_cfg:
                ...
                if extra:
                    title = f"{title} ({' '.join(extra)})"
                create_sensor_card_main(viga2_frame, key, title, r, 0)

        # Panel Central (Total + Diagn├│stico)
        ...
        # Referencia para c├│digo que consulte lbl_tare_value
        try:
            self.lbl_tare_value = None
        except Exception:
            pass

    def _refresh_sensores_tab(self):
        """Re-crea la pesta├▒a SENSORES para reflejar cambios de configuraci├│n."""
        if hasattr(self, '_tab_sensores') and self._tab_sensores.winfo_exists():
            self._setup_sensores_tab(self._tab_sensores)
            ...
            while True:
                ...
                if msg['type'] == 'DATA':
                    ...
                    try:
                        ...
                        pass
                    # A├▒adir al panel de eventos recientes si existe (siempre en memoria)
                    try:
    ...
    def _update_display(self, data):
        # Guardar datos para actualizaci├│n cuando cambie modo decimales
        self._last_sensor_data = data
        # Marcar que recibimos la primera muestra si hay datos ├║tiles
        try:
            ...
            pass
        # Tama├▒os de fuente base para vigas y total (usados en ajuste est├ítico)
        try:
        ...
        if 'total_tare' in data:
            ...
            # Guardar estado actual de tara para l├│gicas de confirmaci├│n
            try:
            ...
            # Actualizar label en pesta├▒a de mantenimiento
            try:
        ...
        # Si acabamos de conectarnos, dar un peque├▒o periodo de gracia antes de
        # mostrar error de comunicaci├│n para que los sensores tengan tiempo de
        ...
        try:
            ...
            if any_disconnected and getattr(self, 'connected', False):
                ...
                # No mostrar error si a├║n estamos dentro del periodo de gracia
                # o si aun no hemos recibido la primera muestra ├║til.
        ...
        if any_disconnected:
            # VERMELHO - H├í sensor(es) desconectado(s) - SISTEMA PARADO
            self.total_section.configure(style='TotalPanelDanger.TFrame')
            self.lbl_total_title.configure(text="ERRO DE COMUNICA├ç├âO", style='TotalLabelDanger.TLabel')
            ...
            # Mantener paridad visual en la pesta├▒a de mantenimiento si existe
            try:
                ...
                if hasattr(self, 'lbl_maint_total_title') and self.lbl_maint_total_title:
                    self.lbl_maint_total_title.configure(text="ERRO DE COMUNICA├ç├âO", style='TotalLabelDanger.TLabel')
            ...
            # Ignoramos la comprobaci├│n 'total_raw>0' para que valores negativos
            ...
            if incoming_total_last > self._widget_last_total:
                ...
                # Ajustar fuente del TOTAL seg├║n decimales y signo negativo
                try:
                    # Derivar tama├▒os base
                    normal_total = self.scaled_font(120)
                except Exception:
                    normal_total = 120
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                # Actualizar tambi├®n la vista de mantenimiento si existe
                try:
                    if hasattr(self, 'lbl_maint_total') and self.lbl_maint_total:
                        try:
            ...
            try:
                if hasattr(self, 'lbl_viga1_total') and self.lbl_viga1_total:
                    ...
                    try:
                        ...
                        # Ajuste est├ítico: usar beam_tgt y ancho expl├¡cito para evitar mover la tarjeta
                ...
                if hasattr(self, 'lbl_viga2_total') and self.lbl_viga2_total:
                    ...
                    try:
                        ...
                        pass
                # Tambi├®n actualizar labels alternativos creados en la nueva UI (compatibilidad)
                try:
        ...
        for key, widgets in list(self.sensor_widgets.items()):
            if key in sensores:
                info = sensores[key]
                # Verificar que el widget de valor a├║n exista (puede pertenecer a un di├ílogo cerrado)
    ...
    def _update_status_led(self, state):
        """Actualiza el color del LED de estado de conexi├│n."""
        led_colors = {
            'connected': ('ÔùÅ', '#22c55e'),  # Verde
            'disconnected': ('ÔùÅ', '#64748b'), # Gris
            'error': ('ÔùÅ', '#ef4444'),       # Rojo
        }
        char, color = led_colors.get(state, ('ÔùÅ', '#64748b'))
        try:
    ...
    def _show_numeric_keypad(self, entry_widget, title="Inserir Valor",
        ...
        # Calcular posici├│n centrada en pantalla
        screen_w = self.winfo_screenwidth()
        screen_h = self.winfo_screenheight()
        x = (screen_w - kp_width) // 2
        y = (screen_h - kp_height) // 2

        # Obtener la ventana padre (puede ser un di├ílogo)
        if entry_widget is not None:
            parent = entry_widget.winfo_toplevel()
        else:
            parent = self
        try:
            # Indicar al watchdog que no eleve el di├ílogo principal mientras exista el keypad
        ...
        # Flag para saber si es la primera pulsaci├│n
        first_press = [True]

        # Variables para modo PIN
        _real_digits = []               # d├¡gitos reales cuando pin_mode=True
        ...
        def press_digit(d):
            if pin_mode:
                ...
                kp_value.set('\u25cf' * len(_real_digits))  # ÔùÅ por d├¡gito
                return
            ...
            if d == "-":
                ...
                first_press[0] = False
                return
            # Si es la primera pulsaci├│n y el valor es "0", reemplazarlo
        ...
        if pin_mode:
            keypad_frame.rowconfigure(0, weight=1)   # t├¡tulo
        ...
        # T├¡tulo personalizado (solo en pin_mode, reemplaza la barra de Windows)
        if pin_mode:
            title_lbl = ttk.Label(
                keypad_frame,
                text=title,
        ...
        def cancel_and_close():
            # _pin_result['value'] permanece None ÔåÆ el caller interpreta como cancelaci├│n
            _close_keypad()

        def _on_physical_key(event):
            """Permite controlar el teclado virtual con teclado f├¡sico."""
            try:
                key = event.keysym or ""
                ch = event.char or ""
    ...
    def _bind_numeric_keypad(self, entry_widget, title="Inserir Valor"):
        """Vincula un Entry para mostrar teclado num├®rico al hacer click."""
        def on_click(event):
            self.after(50, lambda: self._show_numeric_keypad(entry_widget, title))
        entry_widget.bind("<Button-1>", on_click)

    def do_tare(self):
        # Si ya existe una tara aplicada, pedir confirmaci├│n antes de sobrescribir
        try:
        ...
        if has_tare:
            try:
                confirm = self.show_large_confirmation("Confirma├º├úo", "J├í existe uma tara aplicada. Deseja sobrescrever?")
            except Exception:
                confirm = False
            if not confirm:
                return
    ...
    def toggle_decimals(self):
        ...
        Implementaci├│n m├ís segura: evita reentradas, actualiza texto de botones,
        recalcula fuentes usando fuente monoespaciada para n├║meros y libera
        ...
        try:
            ...
            # 3. Forzar actualizaci├│n de datos (el refresco principal reajustar├í las fuentes)
            self._refresh_all_displays()
            
        except Exception as e:
            self.log_message(f"Error toggle_decimals: {e}")
        finally:
            # Liberar bloqueo r├ípidamente
                    ...
                    if hasattr(self.data_processor, 'set_calibration_segments'):
                        for target, pts in cols.items():
                            ...
                            try:
                                ...
                                self.log_message(f"Calibraci├│n aplicada desde {os.path.basename(unified_path)} a target={target} puntos={len(pts)}")
                                applied.add((serial_r, composite_r))
                            except Exception as e:
                                self.log_message(f"Fallo aplicando calibraci├│n unificada para target={target}: {e}")
                    else:
                        ...
                        for target, pts in cols.items():
                            ...
                            try:
                                ...
                                self.log_message(f"Fallo aplicando calibraci├│n unificada (fallback) para target={target}: {e}")
                    ...
                    for (serial_k, composite_k), pts in rows_by_target.items():
                        try:
                            if hasattr(self.data_processor, 'set_calibration_segments'):
                                ...
                                self.log_message(f"Calibraci├│n aplicada desde {os.path.basename(csv_path)} a serial={serial_k} composite={composite_k}")
                            else:
                                self.data_processor.calibration_segments = pts
                                self.data_processor.calibration_method = 'segments'
                                self.log_message(f"Calibraci├│n (fallback) cargada desde CSV para serial={serial_k} composite={composite_k}")
                            applied.add((serial_k, composite_k))
                        except Exception as e:
                            self.log_message(f"Fallo aplicando calibraci├│n CSV para serial={serial_k} composite={composite_k}: {e}")
                    ...
                    if pts:
                        if hasattr(self.data_processor, 'set_calibration_segments'):
                            try:
                                ...
                                self.log_message(f"Calibraci├│n aplicada desde {os.path.basename(path)} a {composite}")
                            except Exception as e:
                                self.log_message(f"Fallo aplicando calibraci├│n {path}: {e}")
                        else:
                            try:
                                self.data_processor.calibration_segments = pts
                                self.data_processor.calibration_method = 'segments'
                                self.log_message(f"Calibraci├│n (fallback) cargada para {composite}")
                            except Exception as e:
                                self.log_message(f"Fallo al establecer calibraci├│n fallback: {e}")
                        break
                except Exception as e:
                    try:
                        self.log_message(f"Error cargando calibraci├│n {path}: {e}")
                    except:
                        pass
    ...
    def _format_weight(self, value):
        ...
        convertible a float, devuelve "0,00" o "0" seg├║n el modo.
        """
        try:
            val = float(value)
        except Exception:
    ...
    def export_calibrations_gui(self):
        """Exporta todas las calibraciones JSON a un ├║nico CSV en el directorio de calibraciones."""
        try:
            from config import CALIBRATIONS_DIR
        except Exception:
            self.log_message("No se encontr├│ CALIBRATIONS_DIR en config.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Exportaci├│n completa", f"Exportadas {len(rows)} filas a:\n{out_path}")
        except Exception as e:
    ...
    def import_calibrations_gui(self, parent=None):
        ...
        try:
            ...
            # Restaurar foco en la ventana padre si se proporcion├│
            if parent:
                try:
                    parent.lift()
                    parent.focus_force()
        ...
        try:
            # Copiar / reemplazar el archivo adecuado seg├║n formato detectado.
                        ...
                        for target, pts in cols.items():
                            ...
                            try:
                                if hasattr(self.data_processor, 'set_calibration_segments'):
                                    ...
                                    self.log_message(f"Calibraci├│n aplicada desde import (curvas) a target={target} puntos={len(pts)}")
                                else:
                                    self.data_processor.calibration_segments = pts
                                    self.data_processor.calibration_method = 'segments'
                                    applied_report.append((target, len(pts)))
                            except Exception as e:
                                self.log_message(f"Fallo aplicando calibraci├│n importada para target={target}: {e}")
                except Exception as e:
            ...
            try:
                if applied_report:
                    ...
                    messagebox.showinfo("Importaci├│n completa", msg)
                else:
                    # Si no aplic├│ nada y no hubo error, mostrar confirmaci├│n de copia
                    if not is_wide:
                        messagebox.showinfo("Importaci├│n", f"CSV importado a:\n{dest}")
                    else:
                        if not applied_report:
                            messagebox.showinfo("Importaci├│n", f"CSV importado a:\n{dest}\nPero no se aplicaron curvas (archivo vac├¡o o valores inv├ílidos).")
            except Exception:
                pass
        except Exception as e:
    ...
    def show_large_confirmation(self, title, message):
        ...
        btn_no = ttk.Button(btn_frame, text="N├âO", bootstyle="danger", width=15, 
    ...
    def _show_reset_confirmation(self):
        resposta = self.show_large_confirmation("Confirma├º├úo", "Tem certeza que deseja limpar a tara?")
        self.log_message(f"Resposta di├ílogo: {resposta}")
        if resposta:
            self.command_queue.put({'cmd': 'RESET_TARE'})
            self.log_message("Tara limpa com sucesso.")
        else:
            self.log_message("Opera├º├úo cancelada.")
    ...
    def _show_sensor_disconnect_dialog(self, payload):
        ...
        # Mensaje principal
        msg_text = f"O sensor '{nombre}' (ID: {node_id}) perdeu a conex├úo.\n\n" \
                   f"A aquisi├º├úo de dados esta pausada."
        msg_lbl = ttk.Label(
            frame, 
            text=msg_text, 
            font=("Segoe UI", 14), 
        ...
        progress_lbl = ttk.Label(
            progress_frame,
            text="Reconex├úo autom├ítica em progresso...",
            font=("Segoe UI", 12),
            foreground="#f59e0b"
        )
        progress_lbl.pack()
        ...
        def on_manual_reconnect():
            ...
            progress_lbl.configure(text="Reconex├úo manual iniciada...", foreground="#22c55e")
        ...
        # Estilo para labels seleccionados en calibraci├│n
        sf = self.scaled_font
    ...
    def _handle_reconnect_failed(self, payload):
        ...
        self.log_message(f" Falha reconex├úo do sensor {node_id} ap├│s {attempts} tentativas")
        ...
        if hasattr(self, '_disconnect_dialogs') and node_id in self._disconnect_dialogs:
            ...
            if dialog_info['progress_label']:
                dialog_info['progress_label'].configure(
                    text=f" RECONEX├âO FALHOU ({attempts} tentativas)",
                    foreground="#dc2626"
                )
            if dialog_info['attempt_label']:
                dialog_info['attempt_label'].configure(
    ...
    def _show_connection_dialog(self):
        ...
        # Criar janela. Si hay un di├ílogo de configuraci├│n abierto, crear
        # este di├ílogo como hijo para evitar conflictos de grab/overrideredirect.
        ...
        try:
            # Se a janela principal ainda n├úo est├í vis├¡vel ou ├® muito pequena, usar a tela
        ...
        # riesgos cuando el parent est├í en overrideredirect/grab.
        try:
            def _try_grab(dlg):
                try:
                    dlg.grab_set()
        ...
        # Enviar primer intento de conexi├│n (cada llamada es una tentativa)
    ...
    def _check_connection_status(self):
        ...
        # Actualizar informaci├│n visual del di├ílogo (sin reintentos autom├íticos por tiempo).
        # El backend notifica el fin de cada tentativa con un mensaje 'STATUS' y
        # la GUI decidir├í si lanzar otra tentativa all├¡.
        try:
            ...
            if status == 'success':
                ...
                if hasattr(self, '_conn_status') and self._conn_status.winfo_exists():
                    self._conn_status.configure(
                        text=" Conex├úo estabelecida com sucesso!",
                        foreground="#22c55e"
                    )
                ...
                # Recuperaci├│n parcial: mostrar mensaje de advertencia y permitir reintento
                if hasattr(self, '_conn_progress') and self._conn_progress.winfo_exists():
                    try:
                        self._conn_progress.stop()
                    except Exception:
                ...
                # Mantener el di├ílogo abierto para que el usuario decida
                ...
                if hasattr(self, '_conn_btn') and self._conn_btn.winfo_exists():
                    # Permitir que el usuario cierre o cancele; no cerrar autom├íticamente
                    self._conn_btn.configure(text="CANCELAR", state='normal')

                # No cerrar el di├ílogo autom├íticamente: la GUI controlar├í reintentos
    ...
    def show_configuration_dialog(self):
        ...
        import json
        import os
        # Re-entrance guard: si el di├ílogo ya est├í abierto, solo levantarlo
        if hasattr(self, '_config_dialog') and self._config_dialog is not None:
            try:
        ...
        # Solicitar PIN de 4 d├¡gitos usando o teclado num├®rico existente
        ...
        if pin is None:
            return  # Usu├írio cancelou
        pwd = pin

        # Contrase├▒a b├ísica hardcodeada (sin gesti├│n adicional)
        if not pwd or pwd != '2847':
            try:
                # Usar el di├ílogo grande de alerta si est├í disponible
        ...
        dialog = ttk.Toplevel(self)
        # Registrar referencia para que otros di├ílogos (p.ej. conexi├│n)
        ...
        if small_screen:
            # TABLET: sin barra de t├¡tulo, pantalla completa
            dialog.overrideredirect(True)
            dialog.geometry(f"{screen_w}x{screen_h}+0+0")
        else:
            # No usar transient() porque en Windows impide que state('zoomed')
            # respete la barra de tareas. La relaci├│n padre-hijo se mantiene
            # mediante el polling de estado (_check_restore_cfg).
            # Maximizar correctamente respetando la barra de tareas
            dialog.state('zoomed')

            # Restaurar jerarqu├¡a al hacer minimize/restore desde la barra de tareas.
            # El evento <Map> no es confiable en Windows para restaurar desde taskbar,
            # as├¡ que usamos un polling ligero que detecta el cambio de estado.
            self._cfg_last_main_state = self.state()
            def _check_restore_cfg():
                try:
                    if not dialog.winfo_exists():
                        return  # Dialog cerrado, detener polling
                    current_state = self.state()
                    prev_state = getattr(self, '_cfg_last_main_state', current_state)
                    self._cfg_last_main_state = current_state
                    # Si la ventana principal pas├│ de iconic/withdrawn a normal/zoomed
                    if prev_state in ('iconic', 'withdrawn') and current_state in ('normal', 'zoomed'):
                        try:
                            dialog.deiconify()
                        except Exception:
                            pass
                        try:
                            ...
                            pass
                    self.after(250, _check_restore_cfg)
                except Exception:
                    pass
            self.after(500, _check_restore_cfg)
        ...
        # T├¡tulo e ├¡cone da janela de configura├º├úo (vis├¡vel na barra de tarefas em modo desktop)
        try:
            dialog.title("Configura├º├Áes do Sistema de Pesagem")
        except Exception:
            pass
        try:
            import os, sys
        ...
        def safe_close_dialog():
            ...
            try:
                ...
                pass
            # Limpiar referencias a botones del di├ílogo para evitar referencias muertas
            try:
                ...
                pass
            # Limpiar referencia al di├ílogo de configuraci├│n
            try:
                ...
                pass
            # El polling de estado (_check_restore_cfg) se detiene autom├íticamente
            # al detectar que el dialog fue destruido.

        # === BORDA para delimitar a janela ===
        # En pantallas peque├▒as eliminamos el padding de la "borda" para
        # evitar un peque├▒o borde visible entre el main y la barra de botones.
        try:
        ...
        # Permitir que main_frame se expanda para que las pesta├▒as puedan usar
        # toda la altura disponible (reservando espacio inferior para la barra de botones).
        main_frame.pack(fill=BOTH, expand=True)
        
        # Variable para almacenar referencia a save_config (se define despu├®s)
        ...
        if small_screen:
            ...
            try:
                # Reservar m├ís altura para los botones en tablet para aprovechar
                # el espacio vac├¡o y evitar que queden pegados al borde inferior.
            ...
            btn_bottom_frame.pack(fill=X, side=BOTTOM, pady=(2, 2))
        ...
        # Buttons size adjustments for small screens (m├ís compactos)
        if small_screen:
            btn_width = 14
            btn_padding = (14, 8)
        else:
        ...
        try:
            # Mostrar coma en el di├ílogo tambi├®n
        ...
        # Guardar propiedades originales para restaurar (evitar cambios de tama├▒o)
        try:
        ...
        # A├▒adir logo2 junto al bot├│n FECHAR (a la derecha)
        try:
            import os, sys
            base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            assets_path = os.path.join(base_path, "assets")
        ...
        # Nota: el logo2 se mostrar├í junto al bot├│n FECHAR en la esquina inferior derecha.

        # Notebook/Tabs (compacto en alto)
        notebook = ttk.Notebook(header_frame, style='BigTab.TNotebook')
        # Permitir que el notebook se expanda verticalmente para que las pesta├▒as
        # (p. ej. SENSORES) puedan usar toda la altura disponible.
        notebook.pack(fill=BOTH, expand=True)

        # ==================== Tab Configura├º├úo ====================
        tab_nodes = ttk.Frame(notebook, padding=10)
        notebook.add(tab_nodes, text="  CONFIGURA├ç├âO  ")
        
        
        # === Porta Serial (USB Gateway) - Compacto ===
        base_font = ("Segoe UI", self.scaled_font(14))
        base_font_bold = ("Segoe UI", self.scaled_font(14), "bold")

        port_frame = ttk.Labelframe(tab_nodes, text="Porta Serial (Gateway USB)", padding=(15, 6), borderwidth=self.scaled(3), relief='solid', labelanchor='n', style='Viga.TLabelframe')
        ...
        ttk.Label(port_grid, text="Porta COM:", font=base_font).grid(row=0, column=0, sticky="w", padx=(0, 15), pady=4)
        
        # Funci├│n para escanear puertos COM disponibles
        def _scan_com_ports():
            com_vals = []
            try:
        ...
        entry_serial = ttk.Combobox(port_grid, font=base_font, values=_scan_com_ports(), width=12)
        entry_serial.set(current_port)
        entry_serial.grid(row=0, column=1, sticky="w", ipady=self.scaled(4), pady=4)
        
        # Bot├│n para refrescar puertos COM disponibles
        def _refresh_com_ports():
            new_vals = _scan_com_ports()
            entry_serial['values'] = new_vals
        
        btn_refresh = ttk.Button(port_grid, text="Atualizar Portos", command=_refresh_com_ports,
        ...
        # Si estamos en pantalla peque├▒a, reservar espacio inferior igual
        ...
        # Tres columnas balanceadas: Descoberta (1) | Viga 1 (1) | Viga 2 (1)
        main_content.columnconfigure(0, weight=1, uniform="cols")
        main_content.columnconfigure(1, weight=1, uniform="cols")
        main_content.columnconfigure(2, weight=1, uniform="cols")
        # Fila ├║nica de contenido principal
        main_content.rowconfigure(0, weight=1)

        # === COLUMNA IZQUIERDA: B├║squeda de Nodos ===
        discover_frame = ttk.Labelframe(main_content, text="Descoberta de N├│s", padding=15, borderwidth=self.scaled(3), relief='solid', labelanchor='n', style='Viga.TLabelframe')
        discover_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5), pady=8)
        try:
        ...
        try:
            style = ttk.Style()
            style.configure('Viga.TLabelframe.Label', font=("Segoe UI", self.scaled_font(14), 'bold'))
        except Exception:
            try:
                style.configure('Viga.TLabelframe.Label', font=("Segoe UI", 14, 'bold'))
            except Exception:
                pass
        ...
        self._discovered_nodes = []
        self._discovered_nodes_var = tk.StringVar(value="Pressione 'Buscar' para descobrir n├│s")
        
        def discover_nodes_action():
            """Inicia descubrimiento de nodos."""
            self._discovered_nodes_var.set(" Procurando n├│s na rede...")
        ...
        ttk.Button(
            ...
            bootstyle="info",
        ).pack(fill=X, ipadx=self.scaled(10), ipady=self.scaled(8))
        ...
        self._disc_tree.heading("node_id", text="ID do N├│")
        self._disc_tree.heading("serial", text="N┬║ S├®rie")
        ...
        self._disc_tree.column("node_id", width=80, minwidth=60, anchor="center", stretch=True)
        self._disc_tree.column("serial", width=100, minwidth=70, anchor="center", stretch=True)
        self._disc_tree.column("channel", width=60, minwidth=50, anchor="center", stretch=True)
        self._disc_tree.column("rssi", width=60, minwidth=50, anchor="center", stretch=True)
        self._disc_tree.column("status", width=80, minwidth=60, anchor="center", stretch=True)
        
        # Bloquear redimensionamiento manual de columnas por el usuario
        try:
            self._disc_tree.bind('<Button-1>', lambda e: 'break' if self._disc_tree.identify_region(e.x, e.y) == 'separator' else None)
        except Exception:
            pass
        
        self._disc_tree.pack(fill=BOTH, expand=True, pady=(10, 0))
        
        # === COLUMNA CENTRAL Y DERECHA: Asignaci├│n de C├®lulas dividida en 2 vigas ===
        viga1 = ttk.Labelframe(main_content, text="Viga 1", padding=15, borderwidth=self.scaled(3), relief='solid', labelanchor='n', style='Viga.TLabelframe')
        viga1.grid(row=0, column=1, rowspan=1, sticky="nsew", padx=(5, 5), pady=8)
        viga1.columnconfigure(0, weight=1)
        viga1.rowconfigure(0, weight=1)
        viga1.rowconfigure(1, weight=1)

        viga2 = ttk.Labelframe(main_content, text="Viga 2", padding=15, borderwidth=self.scaled(3), relief='solid', labelanchor='n', style='Viga.TLabelframe')
        viga2.grid(row=0, column=2, rowspan=1, sticky="nsew", padx=(5, 0), pady=8)
        viga2.columnconfigure(0, weight=1)
        viga2.rowconfigure(0, weight=1)
        viga2.rowconfigure(1, weight=1)
        
        positions = [
            (0, 0, 1, "ATR├üS ESQUERDA"),
            (0, 1, 2, "ATR├üS DIREITA"),
        ...
        for row, col, celda_num, pos_name in positions:
            ...
            # Frame de cada celda con borde m├ís visible, dentro de la viga correspondiente
            parent_viga = viga1 if col == 0 else viga2
            # Mostrar s├│lo la etiqueta simple C├ëLULA N (no mostrar posici├│n frente/atr├ís/izq/der)
            # Layout directamente dentro de la viga (sin Labelframe anidado)
            cell_grid = ttk.Frame(parent_viga)
            cell_grid.grid(row=row, column=0, sticky="nsew", padx=4, pady=4)
            cell_grid.columnconfigure(1, weight=1)
            try:
                cell_grid.columnconfigure(0, weight=0, minsize=self.scaled(60))
            except Exception:
                pass
            
            # T├¡tulo de c├®lula
            ttk.Label(cell_grid, text=f"C├ëLULA {celda_num}", font=base_font_bold, anchor='center').grid(row=0, column=0, columnspan=2, sticky='ew', pady=(0, 6))
            
            # Node ID
            ttk.Label(cell_grid, text="ID:", font=base_font).grid(row=1, column=0, sticky='w', padx=(0, 8), pady=4)
            e_id = ttk.Entry(cell_grid, font=base_font)
            e_id.insert(0, str(current_node_data.get("id", 0)))
            e_id.grid(row=1, column=1, sticky='ew', pady=4)
            self._bind_numeric_keypad(e_id, f"ID do N├│ - C├®lula {celda_num}")
            
            # N├║mero de Serie
            ttk.Label(cell_grid, text="S├®rie:", font=base_font).grid(row=2, column=0, sticky='w', padx=(0, 8), pady=4)
            e_serial = ttk.Entry(cell_grid, font=base_font)
            e_serial.insert(0, str(current_node_data.get("serial", "")))
            e_serial.grid(row=2, column=1, sticky='ew', pady=4)
            self._bind_numeric_keypad(e_serial, f"N┬║ S├®rie - C├®lula {celda_num}")
            
            # Channel - RadioButtons
            ttk.Label(cell_grid, text="Canal:", font=base_font).grid(row=3, column=0, sticky='w', padx=(0, 8), pady=4)
            
            ch_var = tk.StringVar(value=current_node_data.get("ch", "ch1"))
            ch_btn_frame = ttk.Frame(cell_grid)
            ch_btn_frame.grid(row=3, column=1, sticky='w', pady=4)
            
            ch_radio_font = ("Segoe UI", self.scaled_font(15), "bold")
            for ch_opt in ["ch1", "ch2", "ch3"]:
                ...
                ch_btn.pack(side=LEFT, padx=3)
            try:
                for rb in ch_btn_frame.winfo_children():
                    rb.configure(font=ch_radio_font)
            except Exception:
                pass
        ...
        notebook.add(tab_cal, text="  CALIBRA├ç├âO  ")
        
        self._setup_calibration_tab(tab_cal, current_config, safe_close_dialog, dialog)

        # (MANUTEN├ç├âO tab removed - now lives as SENSORES tab in main window)

        # Definir la funci├│n save_config y asignarla a la referencia del header
        def save_config():
            new_config = {
                "execution_mode": "REAL",
                "connection_type": "SERIAL",
            ...
            try:
                ...
                # Recargar configuraci├│n en el DataProcessor para no requerir reinicio
                try:
                    if hasattr(self, 'data_processor') and self.data_processor:
                        try:
                ...
                # Mostrar el mensaje de ├®xito anclado a la ventana de configuraci├│n
                # Registrar y devolver el foco al di├ílogo de configuraci├│n sin mostrar aviso
                try:
                    self.log_message("Configuraci├│n guardada y aplicada.")
                except Exception:
                    pass
                try:
                    dialog.lift()
                    ...
                    pass
                # Refrescar la vista de calibraci├│n para que muestre los nuevos N┬░ de S├®rie
                try:
                    self._refresh_calibration_sensor_serials()
                    try:
                        # Tambi├®n actualizar estilos/selecci├│n visual si la pesta├▒a est├í abierta
                    ...
                    pass
                # Refrescar la pesta├▒a SENSORES de la ventana principal
                try:
                    self._refresh_sensores_tab()
                except Exception:
                    pass

                # Feedback visual: deshabilitar el bot├│n SALVAR durante 5 segundos
                try:
                    # btn_salvar est├í en el scope exterior; cambiar estado y estilo
                    try:
                ...
                try:
                    ...
                    pass
                try:
                    # Mostrar error anclado al di├ílogo de configuraci├│n
                    self.show_alert("Erro", f"N├úo foi poss├¡vel salvar: {e}", "error", parent=dialog)
                finally:
                    try:
                        self._suppress_cfg_watch = False
                    except Exception:
        ...
        # Asignar la funci├│n a la referencia del header
        save_config_ref[0] = save_config

        # Configurar protocolo de cierre
        dialog.protocol("WM_DELETE_WINDOW", safe_close_dialog)

        # NON-MODAL behavior: el di├ílogo de configuraci├│n NO bloquea otros programas.
        # transient(self) ya fue llamado arriba para mantenerlo como hija de la principal.
        # No usar grab_set() ni wait_window() para no bloquear.
        dialog.lift()
        dialog.focus_force()

    def _setup_calibration_tab(self, parent, current_config, close_config_dialog=None, config_dialog=None):
        """Configura a aba de calibra├º├úo de sensores (Layout Tablet Grande)."""
        from modules.calibration import CalibrationManager
        
        # Mapeo de nombres internos a nombres en portugu├®s para display
        def get_display_name(internal_name):
            """Convierte nombre interno a nombre legible en portugu├®s."""
            display_map = {
                'celda_1': 'C├®lula 1',
                'celda_2': 'C├®lula 2', 
                'celda_3': 'C├®lula 3',
        ...
        # Guardar referencia para poder refrescar los n├║meros de serie cuando cambien
        try:
        ...
        if not sensor_names:
            ...
            for name in sensor_names:
                ...
                # permitir que el alto se ajuste seg├║n contenido (no forzar altura fija)
                try:
                ...
                # N├║mero de serie debajo del nombre, si existe
                if serial_number:
                    lbl_serial = ttk.Label(content_frame, text=f"N┬║ Serie: {serial_number}", font=("Segoe UI", 14), foreground="#64748b")
            ...
            def start_calibration_action():
                ...
                # Liberar grab del di├ílogo de config para que wizard funcione
                if config_dialog:
                    try:
            ...
            # Nota: crearemos el bot├│n INICIAR despu├®s de medir los botones secundarios
            # Se defini├│ la funci├│n start_calibration_action arriba; la usaremos al crear INICIAR.
            ...
            # Mantener separaci├│n vertical moderada para evitar empujar los botones de acci├│n abajo
            below_frame.pack(fill=X, pady=(0, self.scaled(20)))

            # Subframe centrado que contiene los botones para evitar que se corten
            # No lo empaquetamos todav├¡a: primero crearemos los botones, mediremos y luego empaquetaremos
            ...
            def export_curves_csv():
                ...
                # Di├ílogo para elegir ubicaci├│n y nombre del archivo
                out_path = None
                try:
                    ...
                    if config_dialog:
                        out_path = filedialog.asksaveasfilename(
                            ...
                            title="Exportar curvas de calibraci├│n",
                    ...
                    # Siempre intentar restaurar la ventana de configuraci├│n
                    if config_dialog:
                        try:
                            config_dialog.lift()
                ...
                # Restaurar ventana de configuraci├│n si existe
                if config_dialog:
                    try:
                        config_dialog.lift()
                        config_dialog.focus_force()
                        ...
                        pass
            # Preparar fuente y estilos para los botones de calibraci├│n (m├ís grandes)
            try:
            ...
            # Crear estilos espec├¡ficos para los botones de calibraci├│n que incluyan la fuente y padding
            try:
            ...
            # Crear funci├│n modal de import/export que muestre SOLO tres botones grandes
            ...
            # Colocar los dos botones en la misma fila, con separaci├│n consistene
            ...
            # Empacar el contenedor para ocupar todo el ancho y alinear cada bot├│n
    ...
    def _update_sensor_buttons_visuals(self, container):
        """Helper para actualizar color de botones de selecci├│n com feedback visual claro."""
        ...
        try:
            ...
            # por eso tambi├®n aplicamos un fallback directo a los widgets m├ís abajo.
            self.style.configure('Card.TFrame', background=BG_CARD, relief='solid', borderwidth=1)
        ...
        for widget in container.winfo_children(): # widget = btn_frame
            ...
            if s_name == selected:
                ...
                for sub in content_frame.winfo_children():
                    if isinstance(sub, ttk.Label):
                        # Aplica estilos personalizados seg├║n el tipo de label
                        try:
                            if hasattr(sub, 'tag') and sub.tag == "name":
                ...
                try:
                    ...
                    pass
                # CR├ìTICO: resetear tambi├®n los atributos directos que se forzaron al seleccionar
    ...
    def _refresh_calibration_sensor_serials(self):
        """Refresca los labels de N┬║ Serie mostrados en la pesta├▒a de calibraci├│n
        a partir de `self.data_processor.nodos_config`. Esto asegura que si el
        usuario cambia el serial en el di├ílogo de configuraci├│n, la vista de
        calibraci├│n lo muestre inmediatamente.
        """
        try:
    ...
    def _open_calibration_wizard(self, current_config, sensor_name_override=None, config_dialog=None):
        """
        Wizard de Calibra├º├úo Avan├ºado.
        Permite entrada manual ou captura, m├║ltiplos pontos, e sele├º├úo de curva.
        """
        # Permitir abrir el asistente aunque no haya data_processor conectado
        if not self.data_processor:

            # Crear un mock m├¡nimo para permitir cargar puntos
            class DummyDP:
        ...
        # Guardar referencia al di├ílogo de config para restaurar grab
        ...
        self._cal_manager.load_points()
        # Mostrar si existe el archivo de calibraci├│n
        try:
        ...
        # ...existing code...
        # (mover refresco de tabla y gr├ífico al final del m├®todo, despu├®s de crear los widgets)
        ...
        # Obtener n├║mero de celda y n├║mero de serie para mostrar en el t├¡tulo
        celda_num = "?"
        serial_num = "?"
        if hasattr(self, '_cal_sensor_selected'):
            internal_name = self._cal_sensor_selected.get()
            # Buscar n├║mero de celda (asume formato 'celda_X')
            if internal_name.startswith("celda_"):
                celda_num = internal_name.split("_")[-1]
            # Buscar n├║mero de serie en la configuraci├│n de nodos_config
            try:
                if hasattr(self.data_processor, 'nodos_config'):
                    nodos_cfg = self.data_processor.nodos_config
                    if internal_name in nodos_cfg:
                ...
                pass
        wizard.title(f"Calibra├º├úo ÔÇö C├®lula {celda_num}  |  N┬║ S├®rie {serial_num}")
        w, h = self.winfo_screenwidth(), self.winfo_screenheight()
        _is_tablet = (w == 1280 and h == 800)
        _is_notebook = (h < 900)  # Notebooks: 1366x768, etc.
        if _is_tablet:
            # Tablet: sin barra de t├¡tulo, pantalla completa
            ...
            # No usar transient() porque en Windows impide que state('zoomed')
            # respete la barra de tareas. La relaci├│n padre-hijo se mantiene
            # mediante el polling de estado (_check_restore_wiz).
            _cfg_parent = config_dialog if (config_dialog is not None) else self
            # Maximizar correctamente respetando la barra de tareas
            wizard.state('zoomed')
            # Restaurar wizard cuando el padre se restaura desde minimizar.
            # Usamos polling de estado en vez de <Map> que no es confiable en Windows.
            self._wiz_last_parent_state = _cfg_parent.state() if hasattr(_cfg_parent, 'state') else 'normal'
            def _check_restore_wiz():
                try:
                    if not wizard.winfo_exists():
                        return  # Wizard cerrado, detener polling
                    current_state = _cfg_parent.state() if _cfg_parent.winfo_exists() else 'normal'
                    prev_state = getattr(self, '_wiz_last_parent_state', current_state)
                    self._wiz_last_parent_state = current_state
                    if prev_state in ('iconic', 'withdrawn') and current_state in ('normal', 'zoomed'):
                        try:
                            wizard.deiconify()
                        except Exception:
                        ...
                        try:
                            ...
                            pass
                    self.after(250, _check_restore_wiz)
                except Exception:
                    pass
            self.after(500, _check_restore_wiz)
        ...
        try:
            ...
            pass
        # Suprimir el watchdog de config mientras el wizard est├® activo
        try:
            self._suppress_cfg_watch = True
        except Exception:
            pass
        ...
        def close_wizard():
            ...
            try:
                ...
                pass
            # Restaurar foco en el di├ílogo de configuraci├│n (usar referencia directa del closure)
            _cfg_ref = config_dialog if (config_dialog is not None) else getattr(self, '_config_dialog', None)
            if _cfg_ref:
                try:
                    if _cfg_ref.winfo_exists():
                        # deiconify() necesario si el config dialog est├í en estado withdrawn/iconic
                        try:
                            _cfg_ref.deiconify()
                        except Exception:
                            pass
        ...
        def get_reading_by_unit():
            """Obtiene la lectura seg├║n la unidad seleccionada."""
            ...
            if unit == "t":
                ...
                # Aplicar multiplicador/inversi├│n por sensor (mismo comportamiento que DataProcessor)
                mult = 1.0
                try:
                    cfg = None
                    if hasattr(self.data_processor, 'nodos_config'):
                ...
                # est├ín en toneladas; por tanto `peso` ya est├í en toneladas.
                try:
                    return float(peso)
                except Exception:
                    return 0.0
            ...
            if unit == "mV/V":
                # Conversi├│n aproximada desde RAW bits a mV/V
                try:
        ...
        def cmd_capture():
            ...
            if unit == "mV/V":
                ...
                # Preservar decimales en RAW para calibraci├│n (3 decimales)
                try:
        ...
        def cmd_add_point():
            try:
                ...
                self.show_alert("Erro", "Valores num├®ricos inv├ílidos", "error", parent=wizard)
                
        def cmd_apply_cal():
            # M├®todo fijo: Interpolaci├│n por Segmentos
            points = self._cal_manager.get_points()

            if len(points) < 2:
                self.show_alert("Erro", "S├úo necess├írios pelo menos 2 pontos para calibrar.", "error", parent=wizard)
                return

            # Crear modelo de interpolaci├│n por segmentos
            sorted_points = sorted(points, key=lambda p: p[1])  # Ordenar por lectura

            # Guardar puntos de calibraci├│n para interpolaci├│n
            cal_data = {
                "method": "segments",
            ...
            # Guardar puntos expl├¡citamente al finalizar
            if hasattr(self._cal_manager, 'save_points'):
                self._cal_manager.save_points()
            self.show_alert("Sucesso", f"Calibra├º├úo salva com {len(sorted_points)} pontos.", "success", parent=wizard)
            close_wizard()
        ...
        _hdr_pad = (15, 8) if _is_notebook else (20, 15)
        header_inner = ttk.Frame(header, padding=_hdr_pad)
        header_inner.pack(fill=X)

        # Icono y t├¡tulo
        # Usar el mismo texto que el t├¡tulo de la ventana
        label_titulo = f"Curva da C├®lula {celda_num} (N┬║ S├®rie {serial_num})"
        ttk.Label(header_inner, text=label_titulo,
                  font=("Segoe UI", 18 if _is_notebook else 22, "bold"),
        ...
        btn_close = ttk.Button(header_inner, text="FECHAR", 
                               ...
                               bootstyle="danger",
                               padding=(20, 8) if _is_notebook else (25, 12))
        btn_close.pack(side=RIGHT)
        
        # Main Split
        _main_pad = 8 if _is_notebook else 15
        main = ttk.Frame(wizard, padding=_main_pad)
        ...
        _lf_pad = 8 if _is_notebook else 15
        left = ttk.Labelframe(main, text="  Dados de Calibra├º├úo  ", padding=_lf_pad, bootstyle="info")
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        
        # Input Form
        f_in = ttk.Frame(left)
        f_in.pack(fill=X, pady=(0, 8 if _is_notebook else 15))
        
        ttk.Label(f_in, text="Insira os pontos de calibra├º├úo:", 
                  font=("Segoe UI", 11 if _is_notebook else 12, "bold")).pack(anchor="w", pady=(0, 5 if _is_notebook else 10))
        ...
        def on_reading_enter(event):
            # Al presionar Enter en lectura, agregar punto autom├íticamente si hay datos
            w_str = self._cal_input_weight.get()
            r_str = self._cal_input_reading.get()
            if w_str and r_str:
                cmd_add_point()
        ...
        ttk.Label(f_w, text="Peso Padr├úo (t):", font=("Segoe UI", 11)).pack(anchor="w")
        e_weight = ttk.Entry(f_w, textvariable=self._cal_input_weight, font=("Segoe UI", 13 if _is_notebook else 16))
        e_weight.pack(fill=X, ipady=4 if _is_notebook else 8)
        e_weight.bind("<Return>", on_weight_enter)
        e_weight.bind("<KP_Enter>", on_weight_enter)
        self._bind_numeric_keypad(e_weight, "Peso Padr├úo (t)")
        
        # Bot├│n ADICIONAR debajo de Peso
        btn_add = ttk.Button(f_w, text="ADICIONAR PONTO", 
                   command=cmd_add_point, 
                   bootstyle="success",
                   padding=(15, 8) if _is_notebook else (20, 15))
        btn_add.pack(fill=X, pady=(6 if _is_notebook else 10, 0))
        ...
        e_reading = ttk.Entry(f_r, textvariable=self._cal_input_reading, font=("Segoe UI", 13 if _is_notebook else 16))
        e_reading.pack(fill=X, ipady=4 if _is_notebook else 8)
        ...
        # Bot├│n CAPTURAR debajo de Leitura
        btn_capture = ttk.Button(f_r, text="CAPTURAR", 
                                  command=cmd_capture, 
                                  bootstyle="info",
                                  padding=(15, 8) if _is_notebook else (20, 15))
        btn_capture.pack(fill=X, pady=(6 if _is_notebook else 10, 0))
        
        # Separador
        ttk.Separator(left).pack(fill=X, pady=8 if _is_notebook else 15)
        ...
        # Columna vac├¡a para alinear con botones de eliminar (sin texto)

        # Scrollable table
        from ttkbootstrap.scrolled import ScrolledFrame
        _tbl_h = 150 if _is_notebook else 250
        self._cal_tbl_scroll = ScrolledFrame(left, autohide=False, height=_tbl_h)
        self._cal_tbl_scroll.pack(fill=BOTH, expand=YES, pady=3 if _is_notebook else 5)

        # === RIGHT: Graph & Config ===
        right = ttk.Labelframe(main, text="An├ílise e Ajuste  ", padding=_lf_pad, bootstyle="warning")
        ...
        # Forzar m├®todo internamente (sin mostrar texto)
        self._cal_method_var.set("Interpola├º├úo Segmentos")
        ...
        g_frame.pack(fill=BOTH, expand=YES, pady=5 if _is_notebook else 10)

        # Inicializar gr├ífico de forma segura
        self._cal_fig = None
        self._cal_ax = None
        ...
        if MATPLOTLIB_AVAILABLE:
            try:
                ...
                ttk.Label(g_frame, text="Erro ao inicializar gr├ífico", 
                          font=("Segoe UI", 12)).pack(expand=YES)
        else:
            ttk.Label(g_frame, text="Matplotlib n├úo dispon├¡vel\nInstale com: pip install matplotlib", 
                      font=("Segoe UI", 12), justify="center").pack(expand=YES)

        # Refrescar tabla y gr├ífico con puntos precargados (si existen)
        self._refresh_cal_wizard_table_ui()
        self._update_cal_wizard_graph()
        # Forzar refresco visual del gr├ífico tras inicializaci├│n completa
        ...
        # Unit selector
        # Secci├│n de selecci├│n de unidad OCULTA por requerimiento. La l├│gica y variable se mantienen para posible uso futuro.
        ...
        ttk.Button(right, text="FINALIZAR E APLICAR CALIBRA├ç├âO", 
                   command=cmd_apply_cal, 
                   bootstyle="success",
                   padding=(15, 8) if _is_notebook else (20, 15)).pack(fill=X, pady=(5 if _is_notebook else 10, 0))
    ...
    def _refresh_cal_wizard_table_ui(self):
        ...
        for i, (peso, lectura) in enumerate(points):
            ...
            # Entry Peso - con teclado num├®rico
            ...
            # Entry Leitura - con teclado num├®rico
    ...
    def _cal_remove_point(self, idx):
        """Elimina un punto de calibraci├│n con confirmaci├│n."""
        ...
        # Mostrar confirmaci├│n con el estilo del programa
    ...
    def _update_cal_wizard_graph(self):
        """Actualiza el gr├ífico usando interpolaci├│n lineal por segmentos."""
        ...
        try:
            ...
            # Scatter - puntos m├ís grandes y visibles
            self._cal_ax.scatter(x, y, c='#2563eb', s=80, zorder=5, edgecolors='white', linewidth=1)

            # Interpolaci├│n Lineal por Segmentos (unir puntos con l├¡neas rectas)
            if len(x) >= 2:
                # Dibujar l├¡neas conectando los puntos ordenados
                self._cal_ax.plot(x, y, 'r-', linewidth=2, zorder=4)
                self._cal_ax.set_title(f"Curva de Calibra├º├úo ({len(points)} pontos)",
            ...
            self.log_message(f"Erro atualizando gr├ífico: {e}")
